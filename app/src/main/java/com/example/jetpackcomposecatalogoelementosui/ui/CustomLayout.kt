...
fun MyBasicColumn(
    ...
    Layout(
        modifier = modifier.border(width = 2.dp, color = Color.Blue),
        content = content
    ){
        ...
        val colocables: List<Placeable> = composablesMedibles.map { composableMedible: Measurable -> //Recorremos la lista de composables hijos
            composableMedible.measure( //Acá indicamos las restriciones del padre para cada composable hijo y nos retorna un objeto colocable que ya se puede posicionar en X e Y
                constraints = constraints
            )
        }
        this.layout(//El contenedor tendra las dimesiones del padre
            ...
            colocables.forEach{ colocable: Placeable ->
                ...
                posicionEnY += colocable.height / colocables.size // Aumentamos el acumulador en altura en Y
            }
        }
    }
}
